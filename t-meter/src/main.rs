...
fn ui(frame: &mut Frame) {
    ...
    for i in 0..width {
        if i == time_pos {
            ...
            spans.push(Span::styled(segment_char, Style::default().fg(Color::White)));
        } else {
            spans.push(Span::styled(segment_char, Style::default().fg(Color::DarkGray)));
        }
    }
    ...
    let quote_widget = Paragraph::new(format!("\"{}\"\n~ {}", quote.text, quote.author))
        .style(Style::default().add_modifier(Modifier::ITALIC).fg(Color::Gray))
